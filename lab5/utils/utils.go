...
func PrintConfusionMatrix(initial, predicted []float64) {
	var matrix = make(map[string]map[string]float64)
	for _, value1 := range initial {
		matrix[strconv.Itoa(int(value1))] = make(map[string]float64)
	}
	for index, value1 := range initial {
		matrix[strconv.Itoa(int(predicted[index]))][strconv.Itoa(int(value1))] += 1
	}
	PrintMatrixByMaps(matrix)
}
